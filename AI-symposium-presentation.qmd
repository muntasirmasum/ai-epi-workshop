...
## Setting Up Your Kitchen Before You Cook {.section-divider-gold data-background-color="#EAAA00"}
...
2. Go to Settings → Developer → Enable MCP
3. ClaudeR will auto-configure the connection

. . .

<div class="tip-box">
<strong>Verify it works:</strong> Load a dataset in RStudio, then ask Claude Desktop: "What data do I have loaded?" — if it answers correctly, you're connected!
</div>

<div class="claude-box">
<strong>Troubleshooting:</strong> If the connection fails, restart both RStudio and Claude Desktop. The MCP handshake sometimes needs a fresh start.
</div>
...
**Step 1: Install Node.js** (required runtime)
...
```bash
# Launch and follow the login prompt
claude

# It will open a browser window
# for Anthropic account authentication
```
...
# Ask it to run R analysis
> "Load NHANES data and run a regression
    of BP on alcohol consumption"
```

<div class="tip-box">
<strong>Note:</strong> Claude Code requires a paid Anthropic API plan. Start with Cowork if you're exploring.
</div>
